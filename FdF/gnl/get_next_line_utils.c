...
/*   Updated: 2023/12/27 15:51:37 by jae-kang         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "gnl.h"

char	*free_alll(t_backup **b, char **temp, char *line, int ex)
{
	if (temp)
	{
		if (*temp)
...
t_backup	*update_backup(t_backup **backup)
{
	...
	while ((*backup)->total < (*backup)->save)
		(*backup)->total *= 2;
	new = (char *) malloc((*backup)->total + 1);
	if (new == 0)
		return (0);
...
t_backup	*reset_backup(t_backup **backup)
{
	...
	*backup = (t_backup *) malloc(sizeof(t_backup));
	if (*backup == 0)
		return (0);
	(*backup)->buf = (char *) malloc(BUFFER_SIZE + 1);
	if (!((*backup)->buf))
		return (0);
	((*backup)->buf)[0] = '\0';
	(*backup)->save = 0;
